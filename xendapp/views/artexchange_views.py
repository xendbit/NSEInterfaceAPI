...
'''
    ...
    if resp_json.get('IsSuccessful'):
        ...
        return Response({'detail': 'Data Successfully processed', 'userId': user_id, 'wallet_account': wallet_account_number, 'bank': bank })
    else: # TODO consider using dummy account number and running cron jobs to create accounts later
        return Response({'detail': f'User registration not successful at this time: {resp_json.get("responseMessage")}'}, status=400)
